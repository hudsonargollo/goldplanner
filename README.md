...
| Path | What | Live |
|---|---|---|
| `goldplanner.clubemkt.digital` | Marketing site + shared login | ✅ |
| `goldplanner.clubemkt.digital/hub`, `/task` | Staff ops — this README's original app | ✅ |
| `goldplanner.clubemkt.digital/portal` | Customer-only panel | ✅ |
| `goldplanner.clubemkt.digital/crm` | Lead pipeline, sales, AI copilot | ✅ |
...
[`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md#data-model-d1-hub-goldplanner).**
...
per-Worker checklist (this app is one of three Workers now — `goldplanner-hub`, and secrets
...
`npm run deploy` (the old `wrangler pages deploy dist --project-name=goldplanner-app` script)
still exists and still works — it's the original standalone Pages deployment
(`tasks.goldplanner.clubemkt.digital`), kept alive until that subdomain has a redirect to `/hub` (see
...
worker/
  hub-entry.js              goldplanner-hub's entry — strips /hub or /task, delegates to functions/
  portal-entry.js            goldplanner-portal's entry — strips /portal, delegates to functions/
  crm-entry.js                goldplanner-crm's entry — Hono API routes, delegates auth to functions/;
...
wrangler.toml                 legacy standalone Pages deploy (tasks.goldplanner.clubemkt.digital)
wrangler.worker.toml          goldplanner-hub
wrangler.portal.toml          goldplanner-portal
wrangler.crm.toml             goldplanner-crm
automation/
