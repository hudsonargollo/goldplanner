...
Apps Script (polls every 30 min, 5am–11pm — Google has no "on save" Drive trigger)
  ...
  → POSTs the transcript to https://tasks.goldplanner.clubemkt.digital/api/analyze/auto
        ↓
...
Cloudflare dashboard → your Pages project (`goldplanner-app`) → **Settings → Variables and secrets** → add a **Secret** named `INGEST_TOKEN` with the value from step 1, for the **Production** environment. Redeploy (or it applies on the next deploy).

CLI alternative:
```sh
npx wrangler pages secret put INGEST_TOKEN --project-name goldplanner-app
```
...
```sh
echo "sk-ant-..." | npx wrangler pages secret put ANTHROPIC_API_KEY --project-name goldplanner-app
```
...
3. Set both secrets on Cloudflare and redeploy:
   ```sh
   echo "https://script.google.com/macros/s/XXXX/exec" | npx wrangler pages secret put MEETINGS_WEBAPP_URL --project-name goldplanner-app
   echo "the-same-WEBAPP_TOKEN" | npx wrangler pages secret put MEETINGS_WEBAPP_TOKEN --project-name goldplanner-app
   ```
...
- Each action line must look like: `[Names] Title: Description`
  (e.g. `[Ana Silva, João Souza] Atualizar VSL: Incluir as novas fotos.`).
