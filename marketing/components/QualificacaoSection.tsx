...
export default function QualificacaoSection() {
  ...
  const [introScene, setIntroScene] = useState<0 | 1 | 2>(0);
  const [data, setData] = useState<FormState>(empty);
  const [eliminated, setEliminated] = useState(false);
  const [noCompanyName, setNoCompanyName] = useState("");
  const [noCompanyPhone, setNoCompanyPhone] = useState("");
  const [noCompanySubmitted, setNoCompanySubmitted] = useState(false);
  const [noCompanySubmitting, setNoCompanySubmitting] = useState(false);
  const [noCompanyError, setNoCompanyError] = useState("");
  ...
  // "Não tenho empresa" doesn't fit today's project scope, but it shouldn't
  // be a dead end — capture a WhatsApp so the team can follow up later
  // instead of just showing the door. Separate from submit()/the scored
  // funnel: this never reached a single scored question, so it's its own
  // minimal lead (see POST /crm/api/public/leads' hasCompany===false branch).
  async function submitNoCompany() {
    if (!noCompanyPhone.trim()) return;
    setNoCompanySubmitting(true);
    setNoCompanyError("");
    try {
      const res = await fetch("/crm/api/public/leads", {
        method: "POST",
        ...
        body: JSON.stringify({
          hasCompany: false,
          name: noCompanyName,
          phone: noCompanyPhone,
          ...attribution.current,
        }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json?.error || "Falha ao enviar.");
      setNoCompanySubmitted(true);
    } catch (e) {
      setNoCompanyError(e instanceof Error ? e.message : "Falha ao enviar. Tente novamente.");
    } finally {
      setNoCompanySubmitting(false);
    }
  }
  ...
  return (
    <section id="qualificacao" className="relative bg-ivory pt-6 pb-16 sm:pt-16 sm:pb-24 overflow-hidden">
      ...
      <div className="relative mx-auto max-w-6xl px-6">
        <div className="grid gap-12 lg:grid-cols-[0.85fr_1.15fr] lg:gap-16">
              ...
              <AnimatePresence mode="wait">
                {introScene === 0 ? (
                  <motion.div
                    ...
                    className="flex min-h-[45svh] flex-col justify-center"
                  >
                    <p className="label-tech mb-4">Processo seletivo</p>
                  ...
                  >
                    ...
                    <p className="mt-6 font-mono text-xs tracking-wide text-ink/30">
                      Leva menos de 5 minutos. Sem compromisso.
                    </p>
          ...
          {/* Right — qualification flow. Always visible on desktop (lg+),
              since the pitch and form sit side by side there. On mobile,
              hidden until the tap-driven intro scenes above are dismissed —
              it becomes its own final scene instead of always sitting in
              the scroll below them (see introScene). */}
          <div className={`rounded-2xl surface-paper-raised p-6 sm:p-8 ${introScene < 2 ? "hidden lg:block" : "block"}`}>
            {/* Progress */}
            <div className="mb-6">
                ...
                >
                  {noCompanySubmitted ? (
                    <>
                      ...
                      <h3 className="mt-5 text-xl font-bold text-ink">Recebemos seu contato.</h3>
                      <p className="mt-2 max-w-sm text-sm leading-relaxed text-ink/60">
                        Entraremos em contato pelo WhatsApp assim que fizer sentido para o seu
                        momento.
                      </p>
                    </>
                  ) : (
                    <>
                      ...
                      <p className="mt-2 max-w-sm text-sm leading-relaxed text-ink/60">
                        Mesmo assim, deixe seu WhatsApp — avisamos você assim que fizer sentido.
                      </p>
                      <div className="mt-6 w-full max-w-xs space-y-3 text-left">
                        <Field label="Nome (opcional)">
                          <input
                            ...
                            value={noCompanyName}
                            onChange={(e) => setNoCompanyName(e.target.value)}
                          />
                        </Field>
                        <Field label="WhatsApp">
                          <input
                            ...
                            value={noCompanyPhone}
                            onChange={(e) => setNoCompanyPhone(e.target.value)}
                          />
                        </Field>
                        {noCompanyError && <p className="text-xs text-danger">{noCompanyError}</p>}
                        <button
                          type="button"
                          disabled={!noCompanyPhone.trim() || noCompanySubmitting}
                          onClick={submitNoCompany}
                          className="group inline-flex w-full items-center justify-center gap-2 rounded-lg bg-green px-5 py-3 text-sm font-bold text-ivory transition-all duration-200 hover:bg-green-hover disabled:cursor-not-allowed disabled:opacity-30 disabled:hover:bg-green"
                        >
                          {noCompanySubmitting ? "Enviando…" : "Quero ser avisado"}
                          <ArrowRight className="h-4 w-4 transition-transform duration-200 group-hover:translate-x-0.5" />
                        </button>
                      </div>
                    </>
