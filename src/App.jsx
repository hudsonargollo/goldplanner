...
export default function App() {
  ...
  // Pending meeting-notes review batches for this user (shown in the notifications bell).
  useEffect(() => {
    if (!authed) return;
    api
      .listReviews()
  ...
  // Validate a single meeting batch without touching the rest.
  ...
  // Deep link from email: tasks.goldplanner.clubemkt.digital/?card=<id> opens that card.
  useEffect(() => {
      ...
      <header className="relative z-20 flex h-14 shrink-0 items-center justify-between border-b border-ink/10 bg-clay/80 px-4 backdrop-blur-xl lg:px-5">
        ...
        <div className="hidden items-center gap-4 lg:flex">
          <NotificationsBell
            ...
            onAckReviews={ackReviews}
            onAckOneReview={ackOneReview}
          />
          {userEmail && (
            <button
      ...
      <div className="relative flex flex-1 overflow-hidden">
        ...
        {view !== "board" ? (
          <div className="relative flex min-w-0 flex-1 overflow-hidden">
            ...
            {view === "devlogs" && (
              <DevLogsPanel clients={clients} userEmail={userEmail} onClose={() => navigateTo("board")} />
            )}
