...
export default function AdminPanel({ currentEmail, clients, onClose }) {
  ...
  const [newStepDraft, setNewStepDraft] = useState({ title: "", owner: "goldplanner", category: "" });
  ...
  async function togglePlan(planId) {
    ...
    setNewStepDraft({ title: "", owner: "goldplanner", category: "" });
          ...
          {tab === "analytics" && (
            <>
              ...
              {!analytics ? (
                ...
                <>
                  {(() => {
                    const { page_view: visits, cta_click: ctaClicks, form_start: starts, form_complete: completes } = analytics.totals;
                    const ctaDrop = Math.max(ctaClicks - starts, 0);
                    ...
                    const funnelStages = [
                      ...
                      { key: "ctaClicks", label: 'Clicaram em "Agendar"', value: ctaClicks, pctOf: visits, bar: "bg-action", text: "text-ink" },
                      { key: "ctaDrop", label: "Clicaram mas não iniciaram o formulário", value: ctaDrop, pctOf: ctaClicks, bar: "bg-danger", text: "text-danger" },
                      { key: "starts", label: "Iniciaram o formulário", value: starts, pctOf: ctaClicks, bar: "bg-action", text: "text-ink" },
                      { key: "drop", label: "Abandonaram o formulário", value: drop, pctOf: starts, bar: "bg-danger", text: "text-danger" },
                                ...
                                <table className="w-full text-left text-sm">
                                  <thead>
                                    <tr className="font-mono text-[9px] uppercase tracking-wider text-stone-500">
                                      ...
                                      <th className="px-3 py-2">Clicaram</th>
                                  ...
                                  <tbody>
                                    {days.map((day) => {
                                      const dayClicks = byDay[day].cta_click || 0;
                                      ...
                                      return (
                                        <tr key={day} className="border-t border-ink/[0.06]">
                                          ...
                                          <td className="px-3 py-2 text-ink">{dayClicks}</td>
