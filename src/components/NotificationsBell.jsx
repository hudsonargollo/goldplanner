import { useState, useEffect, useCallback, useMemo, useRef } from "react";
import { motion, AnimatePresence } from "framer-motion";
import {
  Bell,
  Package,
  MessageSquare,
  PartyPopper,
  Undo2,
  Vibrate,
  UserPlus,
  Pencil,
  MessageCircle,
  Sparkles,
  Check,
  Trash2,
} from "lucide-react";
import { api } from "@/lib/api";
import { Avatar } from "@/components/ui";

function relTime(iso) {
  if (!iso) return "";
  const diff = (Date.now() - new Date(iso).getTime()) / 1000;
  if (diff < 60) return "agora";
  if (diff < 3600) return `${Math.floor(diff / 60)}m`;
  if (diff < 86400) return `${Math.floor(diff / 3600)}h`;
  return `${Math.floor(diff / 86400)}d`;
}

const TYPE_META = {
  mention: { icon: MessageSquare, color: "text-stone-400", badge: "@ você" },
  request: { icon: Package, color: "text-warning", badge: null },
  assigned: { icon: UserPlus, color: "text-action", badge: "atribuído" },
  nudge: { icon: Vibrate, color: "text-action", badge: null },
  reviewed: { icon: PartyPopper, color: "text-success", badge: null },
  reopened: { icon: Undo2, color: "text-warning", badge: null },
  comment: { icon: MessageCircle, color: "text-stone-400", badge: "sua tarefa" },
  updated: { icon: Pencil, color: "text-stone-400", badge: "sua tarefa" },
};

// Self-contained: polls its own state so the 60s refresh never re-renders the board.
// Persistent log — items never disappear, only dim from unread → read on ack.
//
// Also folds in pending "tasks generated from meetings" review batches
// (reviews/cards/onEditTask/onDismissTask/onAckReviews/onAckOneReview) as a
// section at the top of the dropdown — this used to be its own auto-popup
// (ReviewPopup, retired) that interrupted the board on every load.
export default function NotificationsBell({
  authed,
  avatarByName,
  onOpenCard,
  refreshSignal,
  reviews,
  cards,
  onEditTask,
  onDismissTask,
  onAckReviews,
  onAckOneReview,
}) {
  const [open, setOpen] = useState(false);
  const [items, setItems] = useState([]);
  const [unreadCount, setUnreadCount] = useState(0);
  const [reviewBusy, setReviewBusy] = useState(null); // review id currently being acked, or "all"
  const rootRef = useRef(null);

  // Ref-based outside-click detection instead of a full-screen backdrop —
  // works regardless of any z-index/stacking-context the bell happens to
  // sit in (e.g. inside the header's own stacking context), and doesn't
  // eat the click that dismisses it, so clicking straight onto another
  // action closes the menu and fires that action in one click.
  useEffect(() => {
    if (!open) return;
    const onPointerDown = (e) => {
      if (rootRef.current && !rootRef.current.contains(e.target)) setOpen(false);
    };
    document.addEventListener("mousedown", onPointerDown);
    return () => document.removeEventListener("mousedown", onPointerDown);
  }, [open]);

  const refetch = useCallback(() => {
    if (!authed) return;
    api
      .getNotifications()
      .then(({ items, unreadCount }) => {
        setItems(items || []);
        setUnreadCount(unreadCount || 0);
      })
      .catch(() => {});
  }, [authed]);

  useEffect(() => {
    if (!authed) return;
    refetch();
    const t = setInterval(refetch, 60000);
    return () => clearInterval(t);
  }, [authed, refetch]);

  // Re-fetch when the parent signals a change (e.g. a new realtime event).
  useEffect(() => {
    if (refreshSignal !== undefined) refetch();
  }, [refreshSignal, refetch]);

  // Render from the LIVE board so edits show instantly and dismissed
  // (deleted) cards drop out on their own. Each review task's id IS the
  // real card id.
  const cardById = useMemo(() => {
    const m = {};
    for (const c of cards || []) m[c.id] = c;
    return m;
  }, [cards]);

  const liveReviews = useMemo(
    () =>
      (reviews || [])
        .map((r) => ({
          ...r,
          tasks: (r.tasks || []).map((t) => cardById[t.id]).filter(Boolean),
        }))
        .filter((r) => r.tasks.length > 0),
    [reviews, cardById]
  );
  const reviewTaskTotal = liveReviews.reduce((n, r) => n + r.tasks.length, 0);

  const openItem = (n) => {
    onOpenCard(n.cardId);
    if (!n.readAt) {
      setItems((prev) => prev.map((x) => (x.id === n.id ? { ...x, readAt: new Date().toISOString() } : x)));
      setUnreadCount((c) => Math.max(0, c - 1));
      api.ackNotifications([n.id]).catch(() => {});
    }
    setOpen(false);
  };

  const markAll = () => {
    setItems((prev) => prev.map((x) => (x.readAt ? x : { ...x, readAt: new Date().toISOString() })));
    setUnreadCount(0);
    api.ackAllNotifications().catch(() => {});
    setOpen(false);
  };

  const editReviewTask = (card) => {
    onEditTask?.(card);
    setOpen(false);
  };

  async function ackOneReview(id) {
    setReviewBusy(id);
    try {
      await onAckOneReview?.(id);
    } finally {
      setReviewBusy(null);
    }
  }

  async function ackAllReviews() {
    setReviewBusy("all");
    try {
      await onAckReviews?.(liveReviews.map((r) => r.id));
    } finally {
      setReviewBusy(null);
    }
  }

  const badgeCount = unreadCount + reviewTaskTotal;

  return (
    <div className="relative" ref={rootRef}>
      <button
        onClick={() => setOpen((v) => !v)}
        className="relative rounded-lg border border-ink/15 p-1.5 text-stone-500 transition-colors hover:border-action/40 hover:text-action"
        title="Notificações"
      >
        <Bell size={15} />
        {badgeCount > 0 && (
          <span className="absolute -right-1.5 -top-1.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-danger px-1 font-mono text-[9px] font-bold text-clay">
            {badgeCount > 9 ? "9+" : badgeCount}
          </span>
        )}
      </button>

      <AnimatePresence>
        {open && (
            <motion.div
              initial={{ opacity: 0, y: -6, scale: 0.97 }}
              animate={{ opacity: 1, y: 0, scale: 1 }}
              exit={{ opacity: 0, y: -6, scale: 0.97 }}
              transition={{ duration: 0.16 }}
              className="absolute right-0 top-full z-40 mt-2 w-96 overflow-hidden rounded-xl surface-2 shadow-xl"
            >
              <div className="flex items-center justify-between border-b border-ink/10 px-4 py-2.5">
                <span className="label-tech">Notificações</span>
                {unreadCount > 0 && (
                  <button
                    onClick={markAll}
                    className="font-mono text-[10px] text-stone-500 transition-colors hover:text-action"
                  >
                    marcar todas
                  </button>
                )}
              </div>

              {liveReviews.length > 0 && (
                <div className="border-b border-ink/10">
                  <div className="flex items-center justify-between px-4 pt-3 pb-1.5">
                    <span className="flex items-center gap-1.5 font-mono text-[10px] font-semibold uppercase tracking-wider text-stone-500">
                      <Sparkles size={11} style={{ color: "#7A5A6E" }} />
                      Tarefas de reuniões
                      <span className="text-stone-400">· {reviewTaskTotal}</span>
                    </span>
                    {liveReviews.length > 1 && (
                      <button
                        onClick={ackAllReviews}
                        disabled={reviewBusy !== null}
                        className="font-mono text-[10px] text-stone-500 transition-colors hover:text-action disabled:opacity-50"
                      >
                        validar todas
                      </button>
                    )}
                  </div>
                  <ul className="max-h-72 space-y-2 overflow-y-auto px-3 pb-3">
                    {liveReviews.map((r) => (
                      <li key={r.id} className="rounded-lg surface-3 px-3 py-2.5">
                        <div className="mb-1.5 flex items-center justify-between gap-2">
                          <div className="min-w-0">
                            <p className="truncate text-[12.5px] font-semibold text-ink">{r.project}</p>
                            {r.meetingTitle && (
                              <p className="truncate font-mono text-[10px] text-stone-400">{r.meetingTitle}</p>
                            )}
                          </div>
                          <button
                            onClick={() => ackOneReview(r.id)}
                            disabled={reviewBusy !== null}
                            title="Validar (marcar como revisado)"
                            className="shrink-0 rounded-md p-1 text-stone-400 transition-colors hover:bg-success/10 hover:text-success disabled:opacity-50"
                          >
                            <Check size={13} />
                          </button>
                        </div>
                        <ul className="space-y-1">
                          {r.tasks.map((card) => {
                            const assignees = card.assignees?.length
                              ? card.assignees
                              : card.assignee
                                ? [card.assignee]
                                : [];
                            return (
                              <li key={card.id} className="group flex items-center gap-1">
                                <button
                                  type="button"
                                  onClick={() => editReviewTask(card)}
                                  title="Editar tarefa"
                                  className="flex min-w-0 flex-1 items-center gap-1.5 rounded px-1.5 py-1 text-left transition-colors hover:bg-ink/[0.05]"
                                >
                                  <span className="min-w-0 flex-1 truncate text-[12px] text-ink">
                                    {card.title}
                                  </span>
                                  {assignees.length > 0 ? (
                                    <Avatar
                                      name={assignees[0]}
                                      src={avatarByName?.[assignees[0]?.trim().toLowerCase()]}
                                    />
                                  ) : (
                                    <span className="shrink-0 font-mono text-[9px] text-stone-400">
                                      sem responsável
                                    </span>
                                  )}
                                </button>
                                <button
                                  type="button"
                                  onClick={() => onDismissTask?.(card.id)}
                                  title="Dispensar (remover do quadro)"
                                  className="shrink-0 rounded p-1 text-stone-400 opacity-0 transition-colors hover:bg-danger/10 hover:text-danger group-hover:opacity-100"
                                >
                                  <Trash2 size={12} />
                                </button>
                              </li>
                            );
                          })}
                        </ul>
                      </li>
                    ))}
                  </ul>
                </div>
              )}

              {items.length === 0 && liveReviews.length === 0 ? (
                <div className="px-4 py-8 text-center font-mono text-[11px] text-stone-400">
                  nenhuma novidade
                </div>
              ) : items.length > 0 ? (
                <ul className="max-h-96 overflow-y-auto">
                  {items.map((n) => {
                    const meta = TYPE_META[n.type] || TYPE_META.mention;
                    const Icon = meta.icon;
                    const unread = !n.readAt;
                    return (
                      <li key={n.id}>
                        <button
                          onClick={() => openItem(n)}
                          className={`relative flex w-full items-start gap-2.5 border-b border-ink/[0.06] px-4 py-3 text-left transition-colors hover:bg-ink/[0.04] ${
                            unread ? "bg-ink/[0.04]" : "opacity-55"
                          }`}
                        >
                          {unread && (
                            <span className="absolute left-1.5 top-1/2 h-1.5 w-1.5 -translate-y-1/2 rounded-full bg-action" aria-hidden />
                          )}
                          <Avatar
                            name={n.fromName}
                            src={avatarByName?.[n.fromName?.trim().toLowerCase()]}
                            size="md"
                          />
                          <span className="min-w-0 flex-1">
                            <span className="flex items-center gap-1.5">
                              <Icon size={11} className={`shrink-0 ${meta.color}`} />
                              <span className="truncate text-[13px] font-semibold text-ink">
                                {n.cardTitle}
                              </span>
                              {meta.badge && (
                                <span className="shrink-0 rounded bg-action px-1 py-px font-mono text-[8px] font-bold uppercase tracking-wider text-clay">
                                  {meta.badge}
                                </span>
                              )}
                            </span>
                            <span className="mt-0.5 block truncate text-xs text-stone-500">
                              <span className="font-medium text-stone-600">
                                {n.fromName?.split(" ")[0]}
                                {n.type === "nudge" ? " chamou você:" : ":"}
                              </span>{" "}
                              {n.text}
                            </span>
                          </span>
                          <span className="shrink-0 font-mono text-[10px] text-stone-400">
                            {relTime(n.createdAt)}
                          </span>
                        </button>
                      </li>
                    );
                  })}
                </ul>
              ) : null}
            </motion.div>
        )}
      </AnimatePresence>
    </div>
  );
}
