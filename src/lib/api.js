...
export const api = {
  ...
  // dev project logs (Hudson-only changelog feed for internal side projects,
  // distinct from the CRM /projects module — see functions/api/dev-logs).
  // Project picker reuses the same `clients` list as CommercialPanel/FinancePanel.
  listDevLogs: (projectId) => req(`/dev-logs?projectId=${encodeURIComponent(projectId)}`),
  createDevLog: (body) => req("/dev-logs", { method: "POST", body }),
  updateDevLog: (id, body) => req(`/dev-logs/${id}`, { method: "PATCH", body }),
  deleteDevLog: (id) => req(`/dev-logs/${id}`, { method: "DELETE" }),
  promoteDevLogTask: (id, body) => req(`/dev-logs/${id}/tasks`, { method: "POST", body }),
