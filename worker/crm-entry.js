...
// goldplanner.clubemkt.digital, no session required (visitors aren't logged in). Scoring
...
app.post("/crm/api/public/leads", async (c) => {
  ...
  // No company yet doesn't fit today's project scope, but it's not a reason
  // to turn someone away empty-handed — capture a WhatsApp so the team can
  // follow up if/when that changes, instead of a dead-end "no". Same
  // public-form status conventions as a real submission below (row lands as
  // "incomplete", score/tier stay unset since this never reached scoring).
  if (body.hasCompany === false) {
    const phone = String(body.phone || "").trim().slice(0, 60);
    if (!phone) return json(c, { error: "WhatsApp é obrigatório." }, 400);
    ...
    const lead = await createLead(c.env.DB, {
      ...
      utm_campaign: body.utmCampaign || null,
      referrer: body.referrer || null,
      qualification: JSON.stringify({ source: "landing_qualification_form", hasCompany: false }),
      score: null,
      tier: null,
      status: "incomplete",
    });
    await logLeadEvent(c.env.DB, lead.id, {
      type: "created",
      payload: { source: "landing_qualification_form", hasCompany: false },
      actorEmail: "public-form",
    });
    return json(c, { leadId: lead.id, eliminated: true }, 201);
  }
...
const LEAD_DELETE_ALLOWED_EMAIL = "hudsonargollo2@gmail.com";
...
app.get("/crm/api/dashboard", async (c) => {
  ...
  // Link-in-bio funnel — clicks on the single go.goldplanner.clubemkt.digital/sbio link
  ...
  return json(c, {
    ...
    leads: pipelineLeads.map((l) => ({
      ...
      status: l.status,
      score: l.score ?? null,
      closer: l.closer_name || l.assigned_closer_email || null,
