...
// ~/.claude/plans/goldplanner-adaptive-onboarding.md for the full design.
...
async function insertSteps(db, planId, projectId, steps) {
  ...
  const stmts = steps.map((s, i) => {
    ...
    return db
      ...
      .bind(id, planId, projectId, s.title, s.description || null, s.owner || "goldplanner", s.category || null, i, Number.isFinite(s.dueOffsetDays) ? s.dueOffsetDays : null, dueDate);
  });
...
{
  "steps": [
    { "title": "...", "description": "...", "owner": "goldplanner|customer", "category": "kickoff|access|content|technical|design|training|launch", "dueOffsetDays": 3 }
  ]
}`;
}
  ...
  try {
    ...
    const steps = rawSteps
      .map((s) => ({
        ...
        owner: s?.owner === "customer" ? "customer" : "goldplanner",
...
export async function addStep(db, planId, { title, description, owner, category, dueOffsetDays }) {
  ...
  await db
    ...
    .bind(
      ...
      title,
      description || null,
      owner === "customer" ? "customer" : "goldplanner",
      category || null,
      orderIndex,
...
export async function updateStep(db, planId, stepId, patch) {
  ...
  if (fields.owner) fields.owner = fields.owner === "customer" ? "customer" : "goldplanner";
